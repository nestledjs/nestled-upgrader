...
function buildNote(record) {
  ...
  if (record.review) note.review = record.review;
  if (Array.isArray(record.verification) && record.verification.length) {
    note.verification = record.verification;
  }
...
export function publishFeed(config, options = {}, root = ROOT) {
  ...
  const records = downstreamRecords(loadUpgrades(root)).filter(
    (record) =>
      ...
      (record.patch?.path || (record.packageReleases || []).length > 0 || Boolean(record.review)),
  );
  if (!records.length) {
