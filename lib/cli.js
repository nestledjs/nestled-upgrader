...
function versionLines(row) {
  ...
  if (version.state === 'absent') {
    return [`⚠ does not depend on the enforcement package at all (template pins ${version.expected})`];
  }
  // Loud, because file comparison cannot see this: the checks themselves are the wrong build.
  return [`⚠ enforcement package is ${version.actual}, template pins ${version.expected}`];
}
