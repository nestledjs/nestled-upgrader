...
function portLines(row) {
  ...
  if (ports.unset.length > 0) {
    // Unset is not neutral: every default lands on block 0, so silence claims the template's ports.
    const claimsBlockZero = ports.unset.filter((p) => ports.block !== 0);
    if (claimsBlockZero.length > 0) {
      lines.push(
        `⚠ ${claimsBlockZero.length} port(s) unset, so they fall back to block 0: ` +
          nameSome(claimsBlockZero.map((p) => `${p.key}→${p.fallsBackTo} (expected ${p.expected})`))
      );
    }
  }
  return lines;
...
function convergenceDetailLines(row) {
  const lines = [...driftLines(row), ...portLines(row)];
