...
// Every flag that is ever read as a bare boolean (`if (args.x)`) rather than consuming a value
// must be listed here. Omitting one here silently swallows the NEXT argument as its value instead
// of erroring — e.g. `fleet-update --pr --dry-run` parsed as `pr: "--dry-run"` (truthy) with
// `dryRun` never set at all, so a request for a dry run ran for real.
const BOOLEAN_FLAGS = new Set(['--dry-run', '--all', '--allow-dirty', '--dry-run-only', '--push', '--check', '--pr']);
...
export async function main(argv) {
  ...
  if (command === 'retire-upgrade-records') return retireRecords(config, args);
...
Commands:
  ...
  retire-upgrade-records (--project <name> | --all) [--dry-run]
...
function versionLines(row) {
  ...
  // Each of these is a different repair, so each says which one it is rather than sharing a
  // catch-all phrasing that would send the reader after the wrong thing.
  if (version.state === 'absent') {
    return [`⚠ declares no dependency on the enforcement package (template runs ${version.expected})`];
  }
  if (version.state === 'no-manifest') {
    return [`⚠ has no package.json, so the enforcement package cannot be checked (template runs ${version.expected})`];
  }
  if (version.state === 'unreadable') {
    return [`⚠ package.json could not be parsed, so the enforcement package is unknown (template runs ${version.expected})`];
  }
  if (version.state === 'template-unreadable') {
    return [`⚠ the template's package.json could not be parsed — enforcement version unverifiable`];
  }
...
function convergenceStatusCmd(config, args) {
  ...
  // Every state that is not ok/ahead/untracked means the reader cannot say what checks a repo runs.
  const staleVersion = result.rows.filter(
    (row) => row.version && !['ok', 'ahead', 'untracked'].includes(row.version.state)
  ).length;
  console.log(
...
function formatLogs(config, args) {
  ...
  for (const item of results) {
    if (item.skipped) {
      ...
      console.log(`${item.project}: REFUSED — ${item.risks.length} line(s) would lose text to a YAML comment`);
      for (const risk of item.risks) {
        console.log(`    ${item.path}:${risk.line} (${risk.key})`);
        console.log(`      kept:    ${risk.kept}`);
        console.log(`      DROPPED: ${risk.dropped}`);
      }
      console.log(`    Fix: wrap the value in double quotes, then re-run.`);
  ...
  const refused = results.filter((item) => item.risks);
  ...
  console.log(`Upgrade log format: ${changed.length} ${changed.length === 1 ? 'ledger' : 'ledgers'} ${verb}, ${missing.length} missing, ${skipped.length} skipped` +
      (refused.length ? `, ${refused.length} refused (would lose text)` : '') + '.');
  // A refusal is a failure, not a note. Reporting it while exiting 0 would let a preflight pass on
  // ledgers that were never normalized, which is the state the refusal exists to surface.
  if (args.check && (changed.length || refused.length)) {
    throw new Error('Upgrade log formatting check failed.');
  }
  if (!args.check && refused.length) {
    throw new Error(`Refused ${refused.length} ledger(s) that would lose text; quote the values shown above.`);
  }
}

function retireRecords(config, args) {
  const results = retireAllUpgradeRecords(config, undefined, {
    dryRun: Boolean(args.dryRun),
    project: args.project,
    all: Boolean(args.all)
  });
  const describe = {
    retired: (row) => `retired → ${path.basename(row.to)}`,
    'would-retire': (row) => `would retire → ${path.basename(row.to)}`,
    'already-retired': () => 'already retired',
    'no-ledger': () => 'no ledger to retire',
    // Named, not silently skipped: this is the one case where doing nothing is the right answer
    // and the reader still needs to know which repos are waiting.
    'not-converged': () => 'SKIPPED — no .nestled/converged-at; retire this during its first pass',
    conflict: (row) => `SKIPPED — ${path.basename(row.to)} already exists; resolve by hand`
  };
  for (const row of results) {
    console.log(`  ${row.project}: ${(describe[row.state] ?? (() => row.state))(row)}`);
  }
  const done = results.filter((row) => row.state === 'retired' || row.state === 'would-retire').length;
  const waiting = results.filter((row) => row.state === 'not-converged').length;
  const conflicts = results.filter((row) => row.state === 'conflict').length;
  console.log(
    `\n${done} ${args.dryRun ? 'would be retired' : 'retired'}, ${waiting} waiting on a first convergence pass` +
      (conflicts ? `, ${conflicts} needing manual resolution` : '') +
      '.'
  );
  if (!args.dryRun && done > 0) {
    console.log('Files are renamed in each working tree, not committed. Review and commit per repo.');
  }
  // A summary that omits the repos needing hand-work reads as finished when it is not -- the same
  // way format-logs printed its refusals and still exited 0.
  if (conflicts > 0) {
    throw new Error(
      `${conflicts} repo(s) already have a ${RETIRED_LOG_NAME} alongside the live ledger; resolve those by hand.`
    );
  }
}
