...
export function findUpgrade(upgrades, id) {
  const matches = upgrades.filter((item) => item.id === id);
  ...
  const downstream = matches.find(
    (item) => item.recordPath && path.basename(item.recordPath).replace(/\.ya?ml$/, '') === id
  );
...
export function summarizeProject(project, upgrades, root = ROOT) {
  ...
  // Log keys that match no upgrade record. A log entry is only ever found by exact id, so an entry
  // filed under any other key is invisible: its upgrade reads as pending forever and is offered
  // again on every run. This is not hypothetical — the forms 0.8.0 rollout was hand-recorded under
  // four different ids across eight repos (`2026-07-24-forms-upgrade`,
  // `2026-07-24-nestled-forms-0.8.0-upgrade`, …) against a note id of
  // `2026-07-22-nestled-forms-upgrade`, because agents keyed on the date the work ran plus a slug
  // of their own. Surfacing orphans is what separates "not done yet" from "done, filed wrong".
