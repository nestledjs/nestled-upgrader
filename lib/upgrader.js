...
function readDeclaredPorts(repoPath) {
  ...
  for (const line of fs.readFileSync(envPath, 'utf8').split('\n')) {
    // [A-Z0-9_]+, not [A-Z_]+: E2E_API_PORT contains a digit, and the narrower class silently
    // skipped it — the reader could not see a key the formula had just started requiring.
    const match = /^\s*([A-Z0-9_]+)\s*=\s*(\d+)\s*$/.exec(line);
