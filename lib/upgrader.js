...
export function initializeUpgradeLog(project, config, root = ROOT) {
  ...
  // guard, for the same reason: rewriting is where the content would be lost. It also must not
  // abort the caller. This runs across every downstream repo during a template promotion, so a
  // single working tree with unresolved conflict markers in it used to stop the whole fleet
  // operation, in a file nothing reads any more.
  let existing;
  try {
    existing = readUpgradeLog(project, root);
  } catch (error) {
    return { project: project.name, unreadable: error.message };
  }
