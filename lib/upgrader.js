...
// libraries via `workspace:*` while the cloneable template pins the published range those libs ship
// under. The package sync (syncPackagesFromPromotion) already reconciles that version, but the raw
// file mirror compares bytes — so without this it reports every app manifest carrying such a ref as
// drift on each promotion (and would transiently write the workspace ref before the sync corrects
...
// (fleet-upstream #103)
function internalLibWorkspaceSubstitutions(devPath, promotionProject) {
  return findInternalLibVersions(devPath, promotionProject).map(({ name, version }) => ({
    find: `"${name}": "workspace:*"`,
...
export function mirrorTemplateFromSource(config, root = ROOT, { dryRun = false } = {}) {
  ...
  // read as drift (fleet-upstream #103). devPath is resolved above.
  const substitutions = [...policySubstitutions, ...internalLibWorkspaceSubstitutions(devPath, project)];
