...
function propertiesList(text, key) {
  ...
  // written, wrong. A key's value runs until the first line that does not end in a backslash.
  const lines = text.split('\n');
  const start = lines.findIndex((line) => line.startsWith(`${key}=`));
  if (start === -1) return null;

  const values = [];
  let line = lines[start].slice(key.length + 1);
  let index = start;
  for (;;) {
    const continues = line.endsWith('\\');
    const value = (continues ? line.slice(0, -1) : line).trim().replace(/,$/, '').trim();
...
export function collectExcludedFileDrift(devPath, sha, templatePath, ignore = {}) {
  ...
  const devPkg = parseJsonish(sourceText('package.json') || '');
  const tmplPkg = parseJsonish(targetText('package.json') || '');
  if (devPkg && tmplPkg) {
    sections.push({
      file: 'package.json',
      field: 'scripts',
      ignored: ignore.scripts || [],
      ...diffMaps(devPkg.scripts, tmplPkg.scripts, ignore.scripts)
    });
  }

  const devTs = parseJsonish(sourceText('tsconfig.base.json') || '');
  const tmplTs = parseJsonish(targetText('tsconfig.base.json') || '');
  if (devTs && tmplTs) {
    const flatten = (cfg) =>
      Object.fromEntries(
    ...
    sections.push({
      ...
      ...diffMaps(flatten(devTs), flatten(tmplTs), ignore.paths)
    });
  }
  ...
  return sections.filter((s) => s.missing.length || s.differing.length || s.targetOnly.length);
}
