...
/** The enforcement package a manifest pins, or null when it declares none. */
function pinnedDoctor(manifestPath) {
...
/**
 * Compare the enforcement package a repo pins against the template's pin.
 *
...
export function enforcementVersion(templatePath, repoPath) {
  const expected = pinnedDoctor(path.join(templatePath, 'package.json'));
  const actual = pinnedDoctor(path.join(repoPath, 'package.json'));
  if (!expected) return { state: 'untracked' };
  if (!actual) return { state: 'absent', expected };
  return { state: actual === expected ? 'ok' : 'drift', expected, actual };
}
