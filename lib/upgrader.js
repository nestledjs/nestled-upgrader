...
/** The enforcement package a manifest declares, or null when it declares none. */
...
export function enforcementVersion(templatePath, repoPath) {
  const expected = resolvedDoctor(templatePath) ?? declaredDoctor(path.join(templatePath, 'package.json'));
  if (!expected) return { state: 'untracked' };

  const actual = resolvedDoctor(repoPath) ?? declaredDoctor(path.join(repoPath, 'package.json'));
  if (!actual) {
    return declaredDoctor(path.join(repoPath, 'package.json'))
