...
/**
 * Retire one repo, or the whole fleet when asked explicitly.
 *
 * Fleet-wide is opt-in because this writes to OTHER repositories' working trees. A downstream
 * agent, working in its own repo, ran this from the upgrader and renamed the ledgers in two
 * unrelated repos that were mid-branch — correct renames, nobody's intent. An unscoped default on
 * a command that mutates ten checkouts is a footgun regardless of how right each rename is.
 */
export function retireAllUpgradeRecords(config, root = ROOT, options = {}) {
  const only = options.project;
  if (!only && !options.all) {
    throw new Error(
      'retire-upgrade-records writes to each repository\'s working tree.\n' +
        'Name one with --project <name>, or pass --all to retire every converged repo.'
    );
  }
  // Both flags is not a narrowing, it is a contradiction: one of the two asks was going to be
  // ignored. Silently letting --project win means a script that meant --all retires a single repo
  // and reports success.
  if (only && options.all) {
    throw new Error(
      'retire-upgrade-records takes --project <name> or --all, not both.\n' +
        `Drop one: --project ${only} retires that repo, --all retires every converged repo.`
    );
  }
...
export function initializeUpgradeLog(project, config, root = ROOT) {
  ...
  // A ledger this tool cannot parse is one it must not rewrite -- the same rule as the truncation
  // guard, for the same reason: rewriting is where the content would be lost, and here one side of
  // an unresolved conflict would go with it.
  //
  // Only a parse failure is caught. An unreadable file, a permissions error, anything else still
  // throws, because those are not "this repo's bookkeeping is messy" -- they mean the tool cannot
  // see the disk it was pointed at, and continuing would produce a confidently wrong answer.
  //
  // The path is reported with the message: the failure this replaces read "Invalid YAML mapping at
  // line 293" and named no file, across a fleet operation touching ten repos.
  const file = logPath(project, root);
  let existing;
  try {
    existing = readUpgradeLog(project, root);
  } catch (error) {
    const detail = error instanceof Error ? error.message : String(error);
    return { project: project.name, file, unreadable: `${file}: ${detail}` };
  }
...
export function initializeAllLogs(config, root = ROOT) {
  const results = downstreamProjects(config).map((project) => ({
    project: project.name,
    log: initializeUpgradeLog(project, config, root)
  }));
  // Say which repo was skipped and why. A silent skip is how this class of problem hides: the
  // operation reports success while one repo's bookkeeping was never touched.
  for (const { log } of results) {
    if (log?.unreadable) console.warn(`  skipped ${log.project}: ${log.unreadable}`);
  }
  return results;
}
...
const PORT_FORMULA = [
  ...
  // The e2e API needs a block of its own for the same reason the dev API does. Every repo already
  // gets its own test DATABASE (POSTGRES_TEST_PORT), but the e2e API defaulted to 3100 everywhere,
  // so two repos running e2e at once collide -- and the harness adopts whatever is listening, which
  // means one repo's specs run against another repo's database. biztobiz found a muzebook_test
  // database in its own Postgres; this is the same collision one layer up.
  { key: 'E2E_API_PORT', base: 3100, stride: 1 },
...
function readDeclaredPorts(repoPath) {
  ...
  for (const line of fs.readFileSync(envPath, 'utf8').split('\n')) {
    // A digit is allowed after the first character but not as it: E2E_API_PORT contains one, and
    // the original [A-Z_]+ silently skipped the key the formula had just started requiring. Keeping
    // the leading character [A-Z_] matches what a shell will actually accept, so the parser is no
    // looser than the format it reads.
    const match = /^\s*([A-Z_][A-Z0-9_]*)\s*=\s*(\d+)\s*$/.exec(line);
