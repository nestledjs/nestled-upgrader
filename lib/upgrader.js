...
export function convergenceStatus(config, root = ROOT) {
  ...
  const rows = downstreamProjects(config).map((project) => {
    ...
    if (!marker) return { project: project.name, state: 'never', repoPath };
    if (!templateHead || !commitExists(templatePath, marker.sha)) {
      return { project: project.name, state: 'unknown', sha: marker.sha.slice(0, 7), date: marker.date };
    }
    const behind = Number(
