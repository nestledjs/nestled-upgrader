...
const ENFORCEMENT_DIRECTORIES = [
  { dir: 'scripts', match: (name) => /^doctor.*\.ts$/.test(name) },
  { dir: 'tools', match: (name) => /^verify-.*\.ts$/.test(name) }
];
...
export function portConformance(repoPath, block) {
  ...
  for (const [key, value] of Object.entries(expected)) {
    if (!(key in declared)) {
      unset.push({ key, expected: value, fallsBackTo: expectedPorts(0)[key] });
      continue;
    }
...
export function convergenceStatus(config, root = ROOT) {
  ...
  const rows = downstreamProjects(config).map((project) => {
    ...
    if (!marker) return { project: project.name, state: 'never', repoPath, drift, exceptions, ports };
