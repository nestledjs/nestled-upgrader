...
export function inlineCommentRisks(source) {
  ...
  for (const [index, line] of lines.entries()) {
    ...
    if (!match) continue;
    const value = match[2];
    // Quoted and block scalars carry the hash safely; only plain scalars lose it.
    if (/^["'|>]/.test(value)) continue;
    const hash = value.search(/\s#/);
    if (hash === -1) continue;
    risks.push({ line: index + 1, key: match[1], kept: value.slice(0, hash).trim(), dropped: value.slice(hash).trim() });
  }
  return risks;
}
