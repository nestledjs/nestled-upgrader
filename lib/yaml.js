...
// Reads a scalar value that may be a quoted string wrapping onto continuation lines. Every site
// that consumes a raw value must go through this — a value left unfolded keeps its dangling quote
// and its continuation lines then reach the parser as bogus mapping or sequence entries.
function resolveScalar(rawValue, lines, state, indent) {
  const quoted = rawValue[0] === '"' || rawValue[0] === "'";
  return parseScalar(
    quoted && quotedScalarEnd(rawValue) === -1 ? foldQuotedScalar(rawValue, lines, state, indent) : rawValue
  );
}
...
function parseSequence(lines, state, indent) {
  ...
  while (state.index < lines.length) {
    ...
    if (keyValue) {
      ...
      // The value of the item's FIRST key is folded here; subsequent keys go through
      // parseMappingEntry() below, which folds too. Missing this one left
      // `- name: "wrapped` unfolded and its continuation line then threw "Invalid YAML sequence".
      item[key] = value ? resolveScalar(value, lines, state, indent) : parseBlock(lines, state, indent + 2);
...
function parseMappingEntry(lines, state, indent, target) {
  ...
  if (blockScalar) {
    ...
    target[key] = resolveScalar(rawValue, lines, state, indent);
  } else {
